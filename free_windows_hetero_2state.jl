...
function surrogate_values(p::FreeWinParams2S)
    u = zeros(p.C, 2, p.H + 1)
    for i in 1:p.C
        for m in 2:p.H+1
            u[i, 1, m] = (1 - p.p_fail[i]) * u[i, 1, m-1] + p.p_fail[i] * u[i, 2, m-1]
            u[i, 2, m] = p.h[i] + u[i, 2, m-1]   # state 2 absorbing under no-repair
        end
    end
    return u
end
...
function repair_gains(p::FreeWinParams2S, u::Array{Float64,3})
    G    = zeros(p.C, 2, p.H + 1)
...
function lambda_adjust(λstar, m, k, p::FreeWinParams2S)
    k == 0  && return Inf
    k >= m  && return 0.0
    return λstar * (p.K / p.H) / (k / m)
end

function simulate_openings(p::FreeWinParams2S, Gnet::Array{Float64,3},
...
function find_lambda_star(p::FreeWinParams2S, Gnet::Array{Float64,3};
                           tol=1e-3, maxit=40)
    λmin, λmax = 0.0, p.H * sum(p.h[i] for i in 1:p.C)
    for _ in 1:maxit
        λ = (λmin + λmax) / 2
...
function simulate_h1(p::FreeWinParams2S, Gnet::Array{Float64,3},
...
function build_trans(p::FreeWinParams2S)
...
function rolling_lp_h2(p::FreeWinParams2S, x_current::Vector{Int},
    ...
    for t in 1:m, i in 1:p.C, s in 1:2, a in 1:2
    ...
    for t in 1:m, i in 1:p.C
        @constraint(model,
            w_t[t] == sum(x[i, s, k, a, 1, t] for s in 1:2, k in 0:K, a in 1:2))
    end
    ...
    for t in 1:m, i in 1:p.C
        @constraint(model,
            sum(x[i, s, k, a, ww, t]
                for s in 1:2, k in 0:K, a in 1:2, ww in 0:1) == 1.0)
    end

    holding = sum(p.h[i] * x[i, 2, k, a, ww, t]
                  for i in 1:p.C, k in 0:K, a in 1:2, ww in 0:1, t in 1:m)
    repair  = sum(p.c_repair[i] * x[i, s, k, 2, ww, t]
                  for i in 1:p.C, s in 1:2, k in 0:K, ww in 0:1, t in 1:m)
...
function simulate_h2(p::FreeWinParams2S; seed=1234)
    Random.seed!(seed)
    k = p.K; total_loss = 0.0; total_cost = 0.0
    x = fill(1, p.C)
    for m in p.H:-1:1
...
function rolling_lp_h3(p::FreeWinParams2S, x_current::Vector{Int},
                        k_current::Int, m::Int)
    m <= 0 && return nothing, nothing, 0.0
...
function simulate_h3(p::FreeWinParams2S; seed=1234)
    Random.seed!(seed)
    k = p.K; total_loss = 0.0; total_cost = 0.0
    x = fill(1, p.C)
    for m in p.H:-1:1
...
function lp_lower_bound(p::FreeWinParams2S)
...
function run_experiment(; seed_params=42, nsim=300)
    println("\n" * "="^80)
    println("FREE WINDOWS ONLY — 2 STATES (good / failed)")
    ...
    p = FreeWinParams2S(
        C        = C, H = H, K = K,
        p_fail   = rand(rng, C) .* 0.25 .+ 0.05,
        p_repair = fill(1.0, C),
        h        = rand(rng, C) .* 2.00 .+ 0.50,
...
end # module FreeWindowsHetero2S
