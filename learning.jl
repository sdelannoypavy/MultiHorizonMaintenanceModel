...
function main()
    # Hyperparameters
    lr_start = 0.001
    nb_samples = 2 # number of perturbed samples
    ε = 0.1  # scale of the perturbation
    Nb_epochs = 2 
    training_data_nb = 20
    T = 10
    ...
    model = Chain(
        Dense(C + 2 => C + 1, tanh; bias=true, init=zeros),          
    ) # input: degradations + k + T, output: theta_c + theta_k
    ...
    for (x, k, T, m, κ) in train_data
        ...
        output = model(input)
        l += loss(output, y; H, x, k, L, Q, nb_scen)
    end
    push!(loss_list, l)
...
# pourquoi la loss renvoie des valeurs négatives ?
# verifier que l'optim ne donne pas une erreur à chaque fois
# pas sûre de la partie training