...
function V(T,s_init,q_i,S,h,nb_state,P,Q,Vals,d,P_evac)
    ...
    @constraint(model, [s in 1:S, t in 2:(T+1), i in 1:nb_state], x[s,t,i] == (1-ong_m[s,t-1])*sum(P[i,j,2]*x[s,t-1,j] for j in 1:nb_state) + ong_m[s,t-1]*sum(P[i,j,1]*x[s,t-1,j] for j in 1:nb_state))
